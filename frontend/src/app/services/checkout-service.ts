...
import { Checkout, CheckoutError } from '../models/checkout';
...
export class CheckoutService {
  ...
  saveCheckout(checkout: any): Observable<void | CheckoutError> {
    const url = this.baseUrl + '/checkout';
    return this.http.post<void>(url, checkout);
  }

  deleteCheckout(checkOutId: string): Observable<void | CheckoutError> {
    const url = this.baseUrl + '/checkout';
    const params = new HttpParams().set('checkOutId', checkOutId);
