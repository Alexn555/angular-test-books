...
import { Book, BooksError, BookError } from '../models/book';
...
export class BookService {
  ...
  getBooks(filter: Partial<PageRequest>): Observable<Page<Book> | BooksError<Book>> {
  ...
  saveBook(book: Book): Observable<void | BookError> {
    const url = this.baseUrl + '/saveBook';
    return this.http.post<void>(url, book);
  }

  deleteBook(bookId: string): Observable<void | BookError> {
